    ...
    <section id="credentials" className="section bg-midnight py-12 overflow-hidden relative">
      ...
      <div className="container px-6 mx-auto relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white font-outfit">Our <span className="text-royal-gold">Success Stories</span></h2>
          <p className="text-slate-400 mt-6 max-w-xl mx-auto font-inter text-sm">
            Real people, real results. Every placement we make is a milestone — from aspiring candidates to thriving professionals across the globe. These are the journeys that define Career Enterprises.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map((card, idx) => (
            <motion.div
              ...
              className="group cursor-pointer bg-black rounded-[37px] overflow-hidden shadow-2xl shadow-black/40 group-hover:shadow-royal-gold/20 transition-all duration-700"
            >
              <div className="aspect-[1.6/1] bg-black overflow-hidden flex items-center justify-center">
                <img
                  src={card.img}
                  alt={card.title}
              ...
              <div className="px-5 py-5">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <ShieldCheck size={16} className="text-royal-gold" />
                </div>
                <h3 className="text-xl font-black text-white font-outfit tracking-tight mb-2 group-hover:text-royal-gold transition-colors">{card.name}</h3>
                <p className="text-slate-300 text-xs leading-relaxed font-inter">{card.desc}</p>
              </div>
            </motion.div>
          ))}
